...
namespace detail {
                                          ...
                                          , image& angle )
    {
        ...
        for ( std::size_t r = 1 ; r < gss_img.height() - 1 ; ++r ) {
            ...
            for ( std::size_t c = 1 ; c < gss_img.width() - 1 ; ++c ) {
                ...
                if ( derivative_ptr[c] == 0 ) {
                    angle_ptr[c] = y == 0 ? 0 : 90;
                } else {
                                 ...
                                 , image& sup )
    {
        for ( std::size_t r = 1 ; r < derivative.height() - 1 ; ++r ) {
            const double* derivative_ptr_prev = derivative.ptr(r);
            const double* derivative_ptr      = derivative.ptr(r);
            const double* derivative_ptr_next = derivative.ptr(r);
...
void canny_edge( const image& src , image& dst , uint8_t threshold_hi , uint8_t threshold_lo ){
    ...
    codex::vision::gaussian( src , gss_img , 5 );
    ...
    detail::sobel_derivative_and_calc_angle( gss_img , derivative , angle );
